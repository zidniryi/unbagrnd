//! The catalog of background-removal models callers can choose from, plus
//! downloading and caching whichever ones they've selected.
//!
//! This is the only module in this crate that ever makes a network request,
//! and it only does so lazily: the first time a given model is actually
//! needed. Every call after that reads the cached file straight from disk.
//!
//! Unlike the rest of this crate, nothing here is tied to any particular
//! host (Tauri app, HTTP server, CLI, ...): every function takes the
//! model-cache directory and a progress callback explicitly, so callers
//! decide where models live and how (or whether) to surface progress.
...
pub struct ModelSpec {
    /// Stable identifier used by callers (frontend, API request bodies,
    /// settings.json, ...).
    pub key: &'static str,
...
pub fn model_file_path(models_dir: &Path, spec: &ModelSpec) -> PathBuf {
    models_dir.join(spec.file_name)
}

pub fn is_downloaded(models_dir: &Path, spec: &ModelSpec) -> bool {
    model_file_path(models_dir, spec)
        .metadata()
...
pub fn list_models(models_dir: &Path) -> Vec<ModelInfo> {
    MODELS
        ...
        .map(|spec| ModelInfo {
            ...
            downloaded: is_downloaded(models_dir, spec),
        })
        .collect()
}

/// Ensures the given model is present in `models_dir`, downloading and
/// checksum-verifying it if it isn't. Calls `on_progress` as the download
/// proceeds (throttled to roughly 10/sec, plus once on completion) so
/// callers can surface it however fits their host (a Tauri event, a log
/// line, nothing at all).
///
...
pub async fn ensure_model(
    ...
    let final_path = model_file_path(models_dir, spec);
...
pub async fn clear_model(models_dir: &Path, spec: &ModelSpec) -> Result<(), String> {
    let path = model_file_path(models_dir, spec);
    if path.exists() {
